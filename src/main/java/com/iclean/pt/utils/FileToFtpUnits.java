...
public class FileToFtpUnits {
    ...
    public boolean uploadFile(MultipartFile file,String fileName, String romotUpLoadePath) {
...
//        logger.info("============"+localFile);
        ...
        try {
            ...
            success = this.ftpClient.storeFile(fileName, inStream);//上传文件到ftp
            if (success == true) {
                logger.info(fileName + "成功上传到"+romotUpLoadePath);
                return success;
            }
        }catch (FileNotFoundException e) {
            e.printStackTrace();
            logger.info(file + "未找到");
        }catch (IOException e) {
            e.printStackTrace();
        }finally {
            if (null != inStream) {
...
//        String savePicByIo = ftp.savePicByIo(ftp.request, "/");
//        System.out.println("savePicByIo : " + savePicByIo);
        //下载文件夹
//         ftp.delAllFile("D:/mq");
        ftp.ftpLogOut();
    }
