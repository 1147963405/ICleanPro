...
//    public JSONObject getPoints(@RequestBody Map<String,Object> map) {
        logger.info("获取点列表:"+map);
        /*{device_id=181, map_id=}*/
...
//        System.out.println("mapId: "+mapId+ "  deviceId: "+deviceId);
        /*回调：type，name，point*/
        JSONObject jsonObject = commUtil.getJson(map);
        Integer mapId = parseInt(String.valueOf(jsonObject.get("map_id")));
        Integer deviceId = parseInt(String.valueOf( jsonObject.get("device_id")));
        if(mapId==null||deviceId==null){
            return JSONObject.parseObject("{ successed : false , errorCode: 201, msg: 'the mapId or deviceId is null!' ,data:'{}'}");
        }
        List lst=new ArrayList();
...
/*{"devices":[1,2],"map_id":12,"device_id":48}: */
        ...
        * */
        ...
        /*1*/
        StringBuilder append = new StringBuilder().append(" device='" + deviceId + "'");
        DeviceInfoBean deviceInfoBean = deviceService.selectBySelective(append.toString());
        if(deviceInfoBean==null){
            return Result.error().msg("the device is null!");
        }
        /*2*/
...
//                String path = Constants.Global.GLOBAL_STATIC_URL.getValue() + Constants.Global.GLOBAL_MAP_PATH.getValue() + "/" + mapBean.getDeviceId() + "/" + mapBean.getUuid() + "/map.png";
    ...
    public Result downloadMap(String uuid, Integer device_id) {
        ...
        * */
        /*1*/
        StringBuilder append = new StringBuilder().append(" device='" + device_id + "'");
        DeviceInfoBean deviceInfoBean = deviceService.selectBySelective(append.toString());
        if(deviceInfoBean==null){
            return Result.error().msg("the device is bull!");
        }
        /*2*/
